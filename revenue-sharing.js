...
var fs = require('fs');
var arbitrum_rpc = "https://rpc.ankr.com/arbitrum";
...
const NFTcontract = new ethers.Contract(NFTcontractAddress, NFTContractABI, customHttpProviderArbitrum);
...
async function loadHolders(){
    ...
    .then((output) => 
        console.log("Total Earthlings NFT Supply: ",totalSupply = output.toNumber())
    );

    await revenueContract.ve_supply(last_epoch_start_time)
    .then((output) => 
        console.log("xNFTE Supply: ",ve_supply = ethers.utils.formatEther(output))
    );

    await revenueContract.tokens_per_week(last_epoch_start_time)
    .then((output) => 
        console.log("ETH revenue in last Epoch: ",tokens_in_epoch = ethers.utils.formatEther(output))
    );
    ...
    while(i<=totalSupply){
        ...
        if ( ! tokenOwners.includes(tokenOwner) ) {
            ...
            if ( ve_for_at > 0 ) {
                ...
                console.log("$ETH allocated to sddress:  ",Number(eth_share).toFixed(18));
