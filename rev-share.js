...
var fs = require("fs");
var arbitrum_rpc = "https://arbitrum-one.public.blastapi.io";

var customHttpProviderArbitrum = new ethers.providers.JsonRpcProvider(
  arbitrum_rpc
);
...
const xNFTEContractAddress = "0xA7AF63b5154eB5d6Fb50a6d70d5C229e5f030AB2";
const xNFTEContract = new ethers.Contract(
  xNFTEContractAddress,
  xNFTEContractABI,
  customHttpProviderArbitrum
);

const revShareContractABI = [
  {
...
const revShareContractAddress = "0xC15DDD98341346A2d2C9bf0187f56666247dF4C6";
const revShareContract = new ethers.Contract(
  revShareContractAddress,
  revShareContractABI,
  customHttpProviderArbitrum
);
...
console.log(
  "Last Epoch beginning time: ",
  new Date(last_epoch_start_time * 1000).toLocaleDateString("en-US", {
    timeZone: "UTC",
  }),
  new Date(last_epoch_start_time * 1000).toLocaleTimeString("en-US", {
    timeZone: "UTC",
  })
);
...
async function loadHolders() {
  // read method
  await NFTcontract.totalSupply().then((output) =>
    console.log(
      "Total Earthling NFT Supply: ",
      (totalSupply = output.toNumber())
    )
  );

  await revShareContract
    .ve_supply(last_epoch_start_time)
    .then((output) =>
      console.log(
        "xNFTE Supply: ",
        (ve_supply = ethers.utils.formatEther(output))
      )
    );

  await revShareContract
    .tokens_per_week(last_epoch_start_time)
    .then((output) =>
      console.log(
        "ETH revenue in last Epoch: ",
        (tokens_in_epoch = ethers.utils.formatEther(output))
      )
    );

  i = 1;
  while (i <= totalSupply) {
    console.log("Checking Earthling Token ID # ", i);
    let tokenOwner;
    ...
    if (!tokenOwners.includes(tokenOwner)) {
      ...
      await revShareContract
        .ve_for_at(ethers.utils.getAddress(tokenOwner), last_epoch_start_time)
        .then((balance) =>
          console.log(
            "xNFTE Balance: ",
            (ve_for_at = ethers.utils.formatEther(balance))
          )
        );
      ...
      if (ve_for_at > 0) {
        ...
        console.log("Address Percentage Share of RevShare Pool", ve_share * 100);
        console.log("----> Address will receive Rev Share? True. ");
        console.log(
          "$ETH allocated to sddress: ",
          Number(eth_share).toFixed(18)
        );
  ...
  let csv = txList.join("\n");
  fs.writeFile(
    "./airdrop_data/txList" + last_epoch_start_time + ".csv",
    csv,
    "utf8",
    function (err) {
      if (err) {
